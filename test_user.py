...
def register():
    ...
    while True:
        ...
        new_full = {
            "User_name": f"{tmp_user.name}", 
            "Password": f"{tmp_user.password}", 
            "ID": f"{tmp_user.id + 1}",
            "Name": f"{tmp_user.name}", 
            "Email": f"{tmp_user.mail}",
            "Phone_number": f"{tmp_user.phone_num}", 
            "Gender": f"{tmp_user.gender}"}
        write_json(new_full)
